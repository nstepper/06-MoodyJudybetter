$(document).ready(function() {
  ...
  const forecast = document.querySelector('#future-weather'); // Changed from #forecast to #future-weather
  let searchHistory = [];
  ...
  function createCurrentWeatherCard(data) {
      ...
      icon.src = `https://openweathermap.org/img/w/${data.weather[0].icon}.png`; // use backticks
      icon.alt = data.weather[0].description;
      currentWeatherCard.appendChild(icon);

      const temperature = document.createElement('p');
      temperature.innerHTML = `Temperature: ${data.main.temp}&deg;C`; // use backticks
      currentWeatherCard.appendChild(temperature);

      const humidity = document.createElement('p');
      humidity.textContent = `Humidity: ${data.main.humidity}%`; // use backticks
      currentWeatherCard.appendChild(humidity);

      const windSpeed = document.createElement('p');
      windSpeed.textContent = `Wind Speed: ${data.wind.speed} m/s`; // use backticks
  ...
  function createForecastCards(data) {
      ...
      for (let i = 0; i < data.list.length; i += 8) {
          ...
          const icon = document.createElement('img');
          icon.src = `https://openweathermap.org/img/w/${data.list[i].weather[0].icon}.png`; // use backticks
          icon.alt = data.list[i].weather[0].description;
          forecastCard.appendChild(icon);
  
          const temperature = document.createElement('p');
          temperature.innerHTML = `Temperature: ${data.list[i].main.temp}°C`; // use backticks
          forecastCard.appendChild(temperature); // this line was missing in your original code
  ...
  function createSearchHistory(searchTerm) {
      var searchDate = new Date().toLocaleDateString();
      var listItem = $("<li>").text(searchTerm + " - " + searchDate);
      searchHistory.unshift(listItem);
      $('#search-history').prepend(listItem); // added this line to display the search history on the page
  }
});